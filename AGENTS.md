...
- Local Supabase config exists at `supabase/config.toml`. It includes Google Auth config, `site_url = "https://thaoylgvgsvbouyirdfg.supabase.co"`, and redirect allow-list entries for `umassnutrition://auth/callback` and `http://localhost:3000/auth/callback`. The Google OAuth client id is intentionally literal in config so `supabase config push` does not push `env(GOOGLE_CLIENT_ID)` when local env vars are unset; the OAuth client secret must stay out of git.
...
- Backend deployment automation is `.github/workflows/deploy-supabase.yml`: validate Python/mobile checks, push Supabase migrations, deploy `generate-meal-plan` and `chat`, then smoke-test both protected function URLs for `401`. It requires GitHub secrets `SUPABASE_ACCESS_TOKEN` and `SUPABASE_DB_PASSWORD`.
- Frontend deployment automation is `.github/workflows/deploy-frontend.yml`: typecheck the Expo app, export web static files, add a Cloudflare Pages SPA fallback, and deploy to the Cloudflare Pages project `umeal`. It requires GitHub secrets `CLOUDFLARE_API_TOKEN`, `CLOUDFLARE_ACCOUNT_ID`, `EXPO_PUBLIC_SUPABASE_URL`, and `EXPO_PUBLIC_SUPABASE_ANON_KEY`.
